...
data.frame(Date = val.na.temps$dt,
...
plot(na.train.ts, xlim = c(2005,2015),ylab = "Temperature")
...
data.frame(Date = val.eu.temps$dt,
...
plot(eu.train.ts, xlim = c(2005,2015),ylab = "Temperature")
...
data.frame(Date = val.sa.temps$dt,
...
plot(sa.train.ts, xlim = c(2005,2015),ylab = "Temperature")
...
data.frame(Date = val.af.temps$dt,
...
data.frame(Date = val.as.temps$dt,
...
data.frame(Date = val.au.temps$dt,
...
### North America Linear & Neural Net

```{r}
# linear model

na.lm <- tslm(na.train.ts ~ trend + season)
nalm.pred <- forecast(na.lm, h = length(sa.val.ts))
data.frame(Date = val.na.temps$dt,
           Forecast = nalm.pred$mean,
           True = na.val.ts)
...
Plotting results for North America
```{r}
plot(na.train.ts, xlim = c(2005,2015),ylab = "Temperature")
lines(na_nn.pred$fitted, lwd = 2, col = "red")
lines(nalm.pred$fitted, lwd = 2, col = "yellow")
